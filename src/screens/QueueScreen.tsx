...
export function QueueScreen({ filterStatus, title, emptyMessage }: QueueScreenProps) {
  ...
  useEffect(() => {
    (async () => {
      let q = supabase.from('travel_requests').select('*, worksite:worksites(*)').neq('status', 'rascunho');
      ...
      setSegments(sm);
      const trts = await Promise.all(r.map((row) => supabase.from('travel_request_travelers').select('traveler:travelers(full_name)').eq('request_id', row.id).limit(1)));
