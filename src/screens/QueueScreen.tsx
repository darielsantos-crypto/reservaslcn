 ...
 return <div className="space-y-4 min-w-0">
  ...
  {filtered.length===0?<EmptyState icon={<SlidersHorizontal className="h-8 w-8"/>} title="Nenhuma solicitação encontrada"/>:<>
   <div className="md:hidden space-y-2">
    {filtered.map(r=>{const seg=r.segment?.[0];const names=r.travelers?.map((x:any)=>x.traveler?.full_name).filter(Boolean).join(', ')||'—';return <button key={r.id} onClick={()=>navigate(`request/${r.id}`)} className="w-full text-left rounded-2xl border bg-white p-4 shadow-sm active:bg-gray-50">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-medium text-gray-500">{r.request_number}</p><p className="font-semibold text-gray-900 truncate mt-0.5">{names}</p><p className="text-sm text-gray-700 truncate">{seg?`${seg.origin} → ${seg.destination}`:'Trecho não informado'}</p></div><ChevronRight className="h-5 w-5 text-gray-400 shrink-0 mt-1"/></div>
      <div className="grid grid-cols-2 gap-2 mt-3 text-xs text-gray-600"><span className="flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5"/>{formatDateBR(seg?.departure_date)}</span><span className="flex items-center gap-1.5 truncate"><Building2 className="h-3.5 w-3.5 shrink-0"/>{r.worksite?.name||'Sem obra'}</span></div>
