 ...
 return <div className="space-y-4 min-w-0">
  ...
  {filtered.length===0?<EmptyState icon={<SlidersHorizontal className="h-8 w-8"/>} title="Nenhuma solicitação encontrada"/>:<>
   <div className="md:hidden space-y-2">
    {filtered.map(r=>{const seg=r.segment?.[0];const acc=r.accommodation?.[0];const label=seg?`${seg.origin} → ${seg.destination}`:acc?`Hospedagem em ${acc.city}`:'Dados não informados';const date=seg?.departure_date??acc?.check_in;const names=r.travelers?.map((x:any)=>x.traveler?.full_name).filter(Boolean).join(', ')||'—';return <button key={r.id} onClick={()=>navigate(`request/${r.id}`)} className="w-full text-left rounded-2xl border bg-white p-4 shadow-sm active:bg-gray-50">
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-xs font-medium text-gray-500">{r.request_number}</p><p className="font-semibold text-gray-900 truncate mt-0.5">{names}</p><p className="text-sm text-gray-700 truncate">{label}</p></div><ChevronRight className="h-5 w-5 text-gray-400 shrink-0 mt-1"/></div>
      <div className="grid grid-cols-2 gap-2 mt-3 text-xs text-gray-600"><span className="flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5"/>{formatDateBR(date)}</span><span className="flex items-center gap-1.5 truncate"><Building2 className="h-3.5 w-3.5 shrink-0"/>{r.worksite?.name||'Sem obra'}</span></div>
   ...
   <div className="hidden md:block overflow-x-auto rounded-2xl border bg-white mobile-scroll"><table className="w-full min-w-[900px] text-sm"><thead className="bg-gray-50 text-gray-500 text-left"><tr><th className="p-3">Protocolo</th><th className="p-3">Viajante</th><th className="p-3">Trecho</th><th className="p-3">Viagem</th><th className="p-3">Obra</th><th className="p-3">Prazo</th><th className="p-3">Status</th><th className="p-3"></th></tr></thead><tbody>{filtered.map(r=>{const seg=r.segment?.[0];const acc=r.accommodation?.[0];const label=seg?`${seg.origin} → ${seg.destination}`:acc?`Hospedagem em ${acc.city}`:'Dados não informados';const date=seg?.departure_date??acc?.check_in;const names=r.travelers?.map((x:any)=>x.traveler?.full_name).filter(Boolean).join(', ')||'—';return <tr key={r.id} className="border-t hover:bg-gray-50"><td className="p-3 font-medium">{r.request_number}</td><td className="p-3">{names}</td><td className="p-3">{label}</td><td className="p-3">{formatDateBR(date)}</td><td className="p-3">{r.worksite?.name||'—'}</td><td className="p-3"><Badge className={DEADLINE_STYLES[r.deadline_status]}>{DEADLINE_LABELS[r.deadline_status]}</Badge></td><td className="p-3"><Badge className={STATUS_STYLES[r.status]}>{STATUS_LABELS[r.status]}</Badge></td><td className="p-3 text-right"><button className="font-medium text-[#004883] hover:underline" onClick={()=>navigate(`request/${r.id}`)}>Abrir</button></td></tr>})}</tbody></table></div>
  </>}
 </div>
}
