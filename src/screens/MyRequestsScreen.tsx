...
export function MyRequestsScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      r.forEach((row, i) => (accMap[row.id] = (accs[i].data ?? []) as Accommodation[]));
  ...
  const filtered = useMemo(() => {
    return rows.filter((r) => {
      ...
      if (search) {
        ...
        const route = seg ? `${seg.origin} ${seg.destination}` : acc ? `hospedagem ${acc.city ?? ''}` : '';
        const hay = `${r.request_number ?? ''} ${route} ${names[r.id] ?? ''}`.toLowerCase();
        if (!hay.includes(search.toLowerCase())) return false;
      }
      return true;
    });
  }, [rows, status, search, segments, accommodations, names]);
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {filtered.map((r) => (
            <RequestCard
              ...
              accommodations={accommodations[r.id]}
