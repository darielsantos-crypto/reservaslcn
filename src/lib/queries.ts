...
export async function fetchRequestDetail(id: string): Promise<RequestWithRelations | null> {
  ...
  const [ws, reqer, assigned, trt, seg, acc, bag, adv, quo, neg, pur, att, com, hist] =
    await Promise.all([
      req.worksite_id
        ? supabase.from('worksites').select('*').eq('id', req.worksite_id).maybeSingle()
        : Promise.resolve({ data: null }),
      supabase.from('profiles').select('*').eq('id', req.requester_id).maybeSingle(),
      req.assigned_to
        ? supabase.from('profiles').select('*').eq('id', req.assigned_to).maybeSingle()
        : Promise.resolve({ data: null }),
      supabase.from('travel_request_travelers').select('*').eq('request_id', id),
      supabase.from('travel_segments').select('*').eq('request_id', id).order('segment_order'),
      supabase.from('accommodations').select('*').eq('request_id', id),
      supabase.from('baggage_requests').select('*').eq('request_id', id),
      supabase.from('advance_requests').select('*').eq('request_id', id).maybeSingle(),
      supabase.from('quotations').select('*').eq('request_id', id).order('created_at'),
      supabase.from('negotiations').select('*').eq('request_id', id).order('negotiated_at'),
      supabase.from('purchases').select('*').eq('request_id', id).order('created_at'),
      ...
      supabase.from('status_history').select('*').eq('request_id', id).order('created_at'),
    ]);
...
export async function fetchTravelersForWorksite(worksiteId: string | null): Promise<Traveler[]> {
  let q = supabase.from('travelers').select('*').eq('active', true).order('full_name');
...
export async function fetchPolicyRules(): Promise<PolicyRule[]> {
  const { data } = await supabase.from('policy_rules').select('*').order('label');
